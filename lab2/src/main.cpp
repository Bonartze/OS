...
int part = 0; // часть массива над которой работает конкретный поток
int *a;
...
void *merge_sort_tread(void *args) // рекурсивная сортировка по потокам
{
...
void merge_rec(int tread_m) // рекурсивное слияние по потокам
{
